...
batch_size = 32
word_vectors = gensim.models.Word2Vec.load('ko/ko.bin')
...
model.fit([X_char_train, X_train, X_jamo_train, X_pos_train], y_train, batch_size=batch_size, epochs=1,
...
#model1 = load_model('my_model.h5',custom_objects={'ChainCRF'=ChainCRF}) 
#pr = model1.predict([X_char_train, X_train, X_jamo_train, X_pos_train])
#yh = y_train.argmax(2)
#pr = pr.argmax(2)
#fyh, fpr = score(yh, pr)
...
#pr = model1.predict([X_char_test, X_test, X_jamo_test, X_pos_test])
#yh = y_test.argmax(2)
#pr = pr.argmax(2)
#fyh, fpr = score(yh, pr)
